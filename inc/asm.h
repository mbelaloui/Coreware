...
/*   Updated: 2018/10/03 15:32:43 by mbelalou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void						ft_error_label(int error, char *label, char c,
		char *str);
...
void						ft_error_args(int error, char *op, char *args,
		char *arg);
void						ft_error_option(int id_error, char *name,
		t_option *op, char c);
char						*ft_manage_url(char *url);
void						ft_set_asm_options(char c, t_option *options);
BOOL						ft_extract_asm_options(char *param, t_option *op,
		char *name);
...
void						ft_get_arg_translat(t_inst *inst,
		t_symbole *symbole, int index_param);
void						ft_translate_player(t_player *player,
		t_op *op_tab[NBR_OP], t_option *op);
...
BOOL						ft_extract_info_player(t_charlist *file,
		t_player *player, t_op *op_tab[NBR_OP]);
void						ft_put_src(t_player *player, int fd);
void						ft_make_out_put(t_player *player);
char						**ft_prepare_args(char **str, char *name_op,
		t_op *op, char **args);
...
int							ft_extraire_head_info(char *str_file,
		t_player *player);
...
void						ft_get_size_inst(t_inst *inst,
		t_op *op_tab[NBR_OP]);

t_symbole					*ft_new_symbole(char *sym, int add);
int							ft_is_in_symbole(char *symbole, t_symbole *list);
BOOL						ft_add_symbole(char *data, int add,
		t_symbole **list, t_option *op);
...
void						ft_check_for_label(t_symbole *symbole,
		t_instlist *src, t_option *op);
#endif
